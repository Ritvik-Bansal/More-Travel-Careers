...
function DiversityItem({ title, imageUrl }) {
  return (
...
function Diversity() {
  ...
  return (
    <section className="flex flex-col items-start py-20 pl-20 mt-16 w-full bg-red-200 bg-opacity-30 max-md:pl-5 max-md:mt-10 max-md:max-w-full">
      <h2 className="mt-1 ml-4 text-4xl italic font-semibold text-stone-400 max-md:max-w-full">Diversity</h2>
      <div className="flex gap-5 justify-between px-0.5 mt-6 ml-4 text-2xl font-semibold leading-8 text-white max-md:flex-wrap max-md:max-w-full">
        {diversityItems.map((item, index) => (
          <DiversityItem key={index} title={item.title} imageUrl={item.imageUrl} />
        ))}
      ...
      <div className="flex overflow-hidden relative flex-col justify-center items-start self-center mt-14 mb-4 max-w-full border-8 border-solid border-stone-200 min-h-[8px] stroke-[8px] stroke-stone-200 w-[1248px] max-md:pr-5 max-md:mt-10">
        <img loading="lazy" src="https://cdn.builder.io/api/v1/image/assets/TEMP/51e101b73d9c44892df817c0d915a75b862f2a106078808a1e6dadcf6df61927?apiKey=ce6a525162fa4c1692f4c477ef34b183&" alt="" className="object-cover absolute inset-0 size-full" />
        <img loading="lazy" src="https://cdn.builder.io/api/v1/image/assets/TEMP/cfd41620b3726bb3a5b674fab7bf7fa1f8d754ee18c314cbfc6f7cebaf2bc03a?apiKey=ce6a525162fa4c1692f4c477ef34b183&" alt="" className="max-w-full border-8 border-solid aspect-[100] border-stone-300 stroke-[8px] stroke-stone-300 w-[923px]" />
      </div>
    </section>
  );
}
